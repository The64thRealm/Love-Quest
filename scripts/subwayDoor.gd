...
var door_area = preload("res://scenes/subwayDoor.tscn")

func _ready():
	var used_door_tiles = get_used_cells_by_id(15)
	...
	for area in get_tree().get_nodes_in_group("interactAreas"):
		area.connect("body_entered", mapscene_GUI, "OnSubwayAreaEntered")
		area.connect("body_exited", mapscene_GUI, "OnSubwayAreaExited")
